...
describe('loginUser', () => {
  it('stores tokens when login succeeds', async () => {
    global.fetch.mockResolvedValueOnce({ ok: true, json: async () => ({ access_token: 'a1', refresh_token: 'r1' }) });
  ...
  it('returns error on bad credentials', async () => {
    global.fetch.mockResolvedValueOnce({ ok: false, status: 401, json: async() => ({ detail: 'Invalid credentials' }) });
  ...
  it('returns the server guidance message when DB is not initialized (503)', async () => {
    global.fetch.mockResolvedValueOnce({ ok: false, status: 503, json: async() => ({ detail: 'Database not initialized. Run `make init-db` or `alembic upgrade head`' }) });
    const res = await loginUser('x','y');
    expect(res.ok).toBe(false);
